...
F_f = [0.01:0.01:0.08 0.08:0.001:0.10 0.11:0.01:1.0]; F_f = F_f(:);  % Transition between 0.0963-0.0964

% Varying F in the reverse direction
F_r = [0.01:0.01:0.06 0.07:0.001:0.10 0.11:0.01:1.0]; % Transition between 0.088-0.089 but noisy
...
% filename = sprintf('hysteresis_morse');
...
function [Energy,gradE,H] =W(n,F,D,L,x)
    epsilon = 0.00005;                  % REPORT WHAT HAPPENS WHEN EPSILON CHANGES
    [Xj, Xi] = meshgrid(x, x);
    Xij = Xj - Xi;
%   Energy
    e = -Q(L,1,1,Xij) + Q(L,D,F,Xij); % Repulsion + Attraction
...
%   Gradient 
    epout = -Qp(L,1,1,Xij) + Qp(L,D,F,Xij);
...
%   Hessian 
    eppout = -Qpp(L,1,1,Xij) + Qpp(L,D,F,Xij);
...
function diffPotential = Qp(L,D,F,r)
    r2 = r-(L*floor(r/L));
    attract = F*( sinh( ((L/2) - r2)/D )/sinh(L/(2*D))) ;
    attract(abs(r)<eps) = 0;
    diffPotential = attract;
end

function diff2Potential = Qpp(L,D,F,r) 
    r2 = r-(L*floor(r/L));
    attract = - (F/D)*( cosh( ((L/2) - r2)/D )/sinh(L/(2*D))) ;
    attract(abs(r)<eps) = 0;
    diff2Potential = attract;
end
...
%   F = 0.3    ;
    ...
    options = optimoptions('fmincon', 'Algorithm','trust-region-reflective',...
        ...
        'TolX', 1e-16, 'TolFun', 1e-12, 'MaxFunctionEvaluations', 5e4);
    
    count_red = 0;
    figure(1)
    hold on
    for i = 1:length(lambda)
        xs = fmincon(@(input) W(n,lambda(i),D,L,input),x,[],[],[],[],lb,ub,[],options);  % Optimization routine, returns minimum energy configuration
        xs = sort(xs);
        if i == 1
            xold = xs;
        end
        [Energy,gradE,H] = W(n,lambda(i),D,L,xs);   % Retreiving energy, gradient, and hessian information from equilibrium xs
        Energy
        ...
        yline(10, '-');                                 % Plots horizontal line at center of mass of swarm
        xlabel('F','FontSize',18) 
        ylabel('Equilibrium positions','FontSize',18)
        x = 2*xs - xold + (rand(size(x0'))-0.5)*0.001;
        xold = xs;
    end
    count_red
end